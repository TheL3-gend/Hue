...
import { UserIcon, BotIcon, AlertTriangleIcon, CheckCircleIcon, CogIcon, Edit3Icon, FileTextIcon, ZapIcon } from './Icons'; // Assuming Icons.tsx has these
...
const ChatWindow: React.FC<{ messages: Message[]; isLoading: boolean }> = ({ messages, isLoading }) => {
  ...
  const getIconForType = (type?: Message['type']) => {
    switch (type) {
      ...
      case 'status': return <ZapIcon className="w-4 h-4 mr-2 text-cyan-400" />; // Generic status/action
      default: return null;
    }
  };

  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-850"> {/* Slightly different bg for chat */}
      {messages.map((msg) => (
